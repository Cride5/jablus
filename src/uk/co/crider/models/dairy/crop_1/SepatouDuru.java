...
/** The sepatou model as modified by Duru2007 */
public class SepatouDuru {
...
//System.out.println("al:" + al + "\t b:" + b + "\t NI:" + NI + "\t WI:" + WI);
        ...
        // Herbage sensenence
        DMs = 0.85 * DM_rec * Math.min(T, 18) / LLSmin;
...
//System.out.println("DM before:" + DM + "DM after:" + DM_);
			LAI = K_sward * 1.9 * Math.pow(0.01 * DM, 0.73) * (DM / DM_);
			// From Duru2007
			//LAI = 6 * Math.pow(DM/DM_, 0.73) - 1.5;
		}
        else{
...
//        	int LAI_max = 15;
//			LAI += (dLAI / 3) * LAI * (1 - LAI_rec / LAI_max) * (1 - LAI/LAI_max);
        	
        	NI = 0.294f;
			LAI += 0.643 * Math.pow((Math.min(T, 18)), 2) * (NI/100) * WI;;
//			LAI = LAI < 0 ? 0 : LAI;
...
//		    double qoh,     // Amount of offered herbage per cow                           kg     0–2000
	...
	public static void main(String[] args) {
		...
		SepatouDuru sim = new SepatouDuru(
				0, // Day of the year to start at
				SPECIES_COCKSFOOT, // Grass species
				100, // Number of cows in the mob  
...
//				i == 100 || i == 300 || i == 800 ?
//						ACTION_CUT_GRASS :
//				i == 20 ? ACTION_START_GRAZE :
//				i == 50 ? ACTION_STOP_GRAZE : 
//				i == 100 ? ACTION_START_GRAZE :
//				i == 250 ? ACTION_STOP_GRAZE : 
